...
bool Propagator::fast_state_propagate(
  ...
  for (size_t i = 0; i < prop_data.size() - 1; i++) {
    ...
    Eigen::Vector3d w_hat =
        0.5 * (prop_data.at(i + 1).wm + prop_data.at(i).wm) - bias_g;
    Eigen::Vector3d a_hat =
        0.5 * (prop_data.at(i + 1).am + prop_data.at(i).am) - bias_a;
...
std::vector<ov_core::ImuData> Propagator::select_imu_readings(
  ...
  for (size_t i = 0; i < imu_data.size() - 1; i++) {
    ...
    if (imu_data.at(i).timestamp >= time0 &&
      ...
      // PRINT_DEBUG("propagation #%d = CASE 2 =
  ...
  // 1e-3) {
  //    PRINT_DEBUG(YELLOW "Propagator::select_imu_readings(): Missing inertial
  //    measurements to propagate with (%.6f sec missing). IMU-CAMERA are likely
  //    messed up!!!\n" RESET,
...
void Propagator::predict_and_compute(std::shared_ptr<State> state,
  ...
  if (state->_options.do_fej) {
    ...
    F.block(v_id, th_id, 3, 3).noalias() =
        -skew_x(new_v - v_fej + _gravity * dt) * Rfej.transpose();
    ...
    F.block(p_id, th_id, 3, 3).noalias() =
        -skew_x(new_p - p_fej - v_fej * dt + 0.5 * _gravity * dt * dt) *
        Rfej.transpose();
  ...
  Eigen::Matrix<double, 12, 12> Qc = Eigen::Matrix<double, 12, 12>::Zero();
  Qc.block(0, 0, 3, 3) =
      _noises.sigma_w_2 / dt * Eigen::Matrix<double, 3, 3>::Identity();
  Qc.block(3, 3, 3, 3) =
      _noises.sigma_a_2 / dt * Eigen::Matrix<double, 3, 3>::Identity();
  Qc.block(6, 6, 3, 3) =
      _noises.sigma_wb_2 * dt * Eigen::Matrix<double, 3, 3>::Identity();
  Qc.block(9, 9, 3, 3) =
      _noises.sigma_ab_2 * dt * Eigen::Matrix<double, 3, 3>::Identity();
