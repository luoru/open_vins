 ...
 *
 * In general, this class has all the core logic for an Extended Kalman Filter
 * (EKF)-based system. This has all functions that change the covariance along
 * with addition and removing elements from the state. All functions here are
 * static, and thus are self-contained so that in the future multiple states
 * could be tracked and updated. We recommend you look directly at the code for
 * this class for clarity on what exactly we are doing in each and the matching
 * documentation pages.
 */
class StateHelper {
 public:
   ...
   *
   * The mean of the state should already have been propagated, thus just moves
   * the covariance forward in time. The new states that we are propagating the
   * old covariance into, should be **contiguous** in memory. The user only
   * needs to specify the sub-variables that this block is a function of. \f[
   * \tilde{\mathbf{x}}' =
   * \begin{bmatrix}
   * \boldsymbol\Phi_1 &
   * \boldsymbol\Phi_2 &
   ...
   * @param order_NEW Contiguous variables that have evolved according to this
   * state transition
   ...
   * elements. Will also ensure that proper cross-covariances are inserted.
   * @param state Pointer to state
   ...
   * Thus the size of the return will be the summed dimension of all the passed
   * variables. Normal use for this is a chi-squared check before update (where
   * you don't need the full covariance).
   *
   * @param state Pointer to state
   ...
   *
   * Should only be used during simulation as operations on this covariance will
   * be slow. This will return a copy, so this cannot be used to change the
   * covariance by design. Please use the other interface functions in the
   * StateHelper to progamatically change to covariance.
   *
   * @param state Pointer to state
  ...
  /**
   * @brief Marginalizes a variable, properly modifying the ordering/covariances
   * in the state
   *
   ...
   * For example if you wanted to just marginalize the orientation of a PoseJPL,
   * that isn't supported. We will first remove the rows and columns
   * corresponding to the type (i.e. do the marginalization). After we update
   * all the type ids so that they take into account that the covariance has
   * shrunk in parts of it.
   *
   * @param state Pointer to state
   ...
   *
   * Uses Givens to separate into updating and initializing systems (therefore
   * system must be fed as isotropic). If you are not isotropic first whiten
   * your system (TODO: we should add a helper function to do this). If your H_L
   * Jacobian is already directly invertable, the just call the
   * initialize_invertible() instead of this function. Please refer to @ref
   * update-delay page for detailed derivation.
   *
   * @param state Pointer to state
   * @param new_variable Pointer to variable to be initialized
   * @param H_order Vector of pointers in order they are contained in the
   * condensed state Jacobian
   ...
   * This is just the update assuming that H_L is invertable (and thus square)
   * and isotropic noise.
   *
   * @param state Pointer to state
   * @param new_variable Pointer to variable to be initialized
   ...
   * condensed state Jacobian
   * @param H_R Jacobian of initializing measurements wrt variables in H_order
   * @param H_L Jacobian of initializing measurements wrt new variable (needs to
   * be invertible)
   ...
   *
   * After propagation, normally we augment the state with an new clone that is
   * at the new update timestep. This augmentation clones the IMU pose and adds
   * it to our state's clone map. If we are doing time offset calibration we
   * also make our cloning a function of the time offset. Time offset logic is
   * based on Mingyang Li and Anastasios I. Mourikis paper:
   * http://journals.sagepub.com/doi/pdf/10.1177/0278364913515286 We can write
   * the current clone at the true imu base clock time as the follow:
   * \f{align*}{
   * {}^{I_{t+t_d}}_G\bar{q} &= \begin{bmatrix}\frac{1}{2}
   * {}^{I_{t+\hat{t}_d}}\boldsymbol\omega \tilde{t}_d \\
   * 1\end{bmatrix}\otimes{}^{I_{t+\hat{t}_d}}_G\bar{q} \\
   * {}^G\mathbf{p}_{I_{t+t_d}} &= {}^G\mathbf{p}_{I_{t+\hat{t}_d}} +
   * {}^G\mathbf{v}_{I_{t+\hat{t}_d}}\tilde{t}_d \f} where we say that we have
   * propagated our state up to the current estimated true imaging time for the
   * current image, \f${}^{I_{t+\hat{t}_d}}\boldsymbol\omega\f$ is the angular
   * velocity at the end of propagation with biases removed. This is off by some
   * smaller error, so to get to the true imaging time in the imu base clock, we
   * can append some small timeoffset error. Thus the Jacobian in respect to our
   * time offset during our cloning procedure is the following: \f{align*}{
   * \frac{\partial {}^{I_{t+t_d}}_G\tilde{\boldsymbol\theta}}{\partial
   * \tilde{t}_d} &= {}^{I_{t+\hat{t}_d}}\boldsymbol\omega \\ \frac{\partial
   * {}^G\tilde{\mathbf{p}}_{I_{t+t_d}}}{\partial \tilde{t}_d} &=
   ...
   *
   * This will marginalize the clone from our covariance, and remove it from our
   * state. This is mainly a helper function that we can call after each update.
   * It will marginalize the clone specified by State::margtimestep() which
   * should return a clone timestamp.
   *
   * @param state Pointer to state
   */
