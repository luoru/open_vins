...
struct VioManagerOptions {
  ...
  void print_and_load_noise(
    ...
    if (parser != nullptr) {
      ...
      parser->parse_config("up_msckf_chi2_multipler",
                           msckf_options.chi2_multipler);
      parser->parse_config("up_slam_sigma_px", slam_options.sigma_pix);
      parser->parse_config("up_slam_chi2_multipler",
                           slam_options.chi2_multipler);
    ...
    if (parser != nullptr) {
      ...
      if (use_mask) {
        for (int i = 0; i < state_options.num_cameras; i++) {
          ...
          //   total_mask_path.c_str()); std::exit(EXIT_FAILURE);
          // }
  ...
  /// If our front-end should try to use some multi-threading for stereo
  /// matching
  ...
  void print_and_load_trackers(
    ...
    if (parser != nullptr) {
      ...
      if (histogram_method_str == "NONE") {
        ...
        printf(RED
               "VioManager(): invalid feature histogram specified:\n" RESET);
        printf(RED "\t- NONE\n" RESET);
