...
class VioManager {
 public:
  ...
  void feed_measurement_imu(const ov_core::ImuData &message) {
    // Get the oldest camera timestamp that we can remove IMU measurements
    // before Then push back to our propagator and pass the IMU time we can
    // delete up to
    double oldest_time =
        trackFEATS->get_feature_database()->get_oldest_timestamp();
    if (oldest_time != -1) {
  ...
  void initialize_with_gt(Eigen::Matrix<double, 17, 1> imustate) {
    ...
    Eigen::MatrixXd Cov = 1e-4 * Eigen::MatrixXd::Identity(state->_imu->size(),
                                                           state->_imu->size());
    // Cov.block(state->_imu->v()->id(), state->_imu->v()->id(), 3, 3) *= 10;
    // Cov(state->_imu->q()->id() + 2, state->_imu->q()->id() + 2) = 0.0;
    // Cov.block(state->_imu->p()->id(), state->_imu->p()->id(), 3,
    // 3).setZero(); Cov.block(state->_imu->q()->id(), state->_imu->q()->id(),
    // 3, 3) =
    //     state->_imu->Rot() * Cov.block(state->_imu->q()->id(),
    //     state->_imu->q()->id(), 3, 3) * state->_imu->Rot().transpose();
 ...
 protected:
  ...
  /**
   * @brief This function will will re-triangulate all features in the current
   * frame
   *
   * For all features that are currently being tracked by the system, this will
   * re-triangulate them. This is useful for downstream applications which need
   * the current pointcloud of points (e.g. loop closure). This will try to
   * triangulate *all* points, not just ones that have been used in the update.
   *
   * @param message Contains our timestamp, images, and camera ids
   */
  ...
  // Re-triangulated features 3d positions seen from the current frame (used in
  // visualization)
