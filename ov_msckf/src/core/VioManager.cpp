...
VioManager::VioManager(VioManagerOptions &params_)
  ...
  if (params.use_klt) {
    trackFEATS = std::shared_ptr<TrackBase>(new TrackKLT(
        state->_cam_intrinsics_cameras, params.init_options.init_max_features,
  ...
  active_tracks_initializer =
      std::make_shared<FeatureInitializer>(params.featinit_options);
}
...
void VioManager::feed_measurement_simulation(
  ...
  if (!is_initialized_vio) {
    PRINT_ERROR(RED
                "[SIM]: your vio system should already be initialized before "
                "simulating features!!!\n" RESET);
    PRINT_ERROR(RED
                "[SIM]: initialize your system first before calling "
...
void VioManager::do_feature_propagate_update(
  ...
  if (state->_timestamp > message.timestamp) {
    PRINT_WARNING(YELLOW
                  "image received out of order, unable to do anything (prop dt "
                  "= %3f)\n" RESET,
  ...
  std::sort(featsup_MSCKF.begin(), featsup_MSCKF.end(),
  ...
  // time_prop); PRINT_DEBUG(BLUE "[TIME]: %.4f seconds for MSCKF update (%d
  // feats)\n" RESET, time_msckf, (int)featsup_MSCKF.size()); if
  ...
  //   PRINT_DEBUG(BLUE "[TIME]: %.4f seconds for SLAM delayed init (%d
  //   feats)\n" RESET, time_slam_delay, (int)feats_slam_DELAYED.size());
  // }
  // PRINT_DEBUG(BLUE "[TIME]: %.4f seconds for re-tri & marg (%d clones in
  // state)\n" RESET, time_marg, (int)state->_clones_IMU.size());

  // std::stringstream ss;
  // ss << "[TIME]: " << std::setprecision(4) << time_total << " seconds for
  // total (camera"; for (const auto &id : message.sensor_ids) {
  //   ss << " " << id;
  // }
  // ss << ")" << std::endl;
  ...
  PRINT_INFO(
      "q_GtoI = %.3f,%.3f,%.3f,%.3f | p_IinG = %.3f,%.3f,%.3f | dist = %.2f "
      "(meters)\n",
...
void VioManager::retriangulate_active_tracks(
    const ov_core::CameraData &message) {
  // Start timing
  // boost::posix_time::ptime retri_rT1, retri_rT2, retri_rT3, retri_rT4,
  // retri_rT5; retri_rT1 = boost::posix_time::microsec_clock::local_time();
  ...
  // NOTE: This database should have all features from all trackers already in
  // it NOTE: it also has the complete history so we shouldn't see jumps from
  // deleting measurements
  std::vector<std::shared_ptr<Feature>> active_features =
      trackDATABASE->features_containing_older(state->_timestamp);
  ...
  // times
  //    Also remove any that we are unable to triangulate (due to not having
  //    enough measurements)
  auto it0 = active_features.begin();
  while (it0 != active_features.end()) {
    // Skip if it is a SLAM feature since it already is already going to be
    // added
    if (state->_features_SLAM.find((*it0)->featid) !=
        state->_features_SLAM.end()) {
    ...
    // Remove if we don't have enough and am not a SLAM feature which doesn't
    // need triangulation
    if (ct_meas < (int)std::max(4.0, std::floor(state->_options.max_clone_size *
                                                2.0 / 5.0))) {
      it0 = active_features.erase(it0);
    } else {
      it0++;
  ...
  if (active_features.empty() && state->_features_SLAM.empty()) return;
  ...
  for (const auto &clone_calib : state->_calib_IMUtoCAM) {
    ...
    for (const auto &clone_imu : state->_clones_IMU) {
      ...
      Eigen::Matrix3d R_GtoCi =
          clone_calib.second->Rot() * clone_imu.second->Rot();
      Eigen::Vector3d p_CioinG =
  ...
  while (it1 != active_features.end()) {
    ...
    if (active_tracks_initializer->config().triangulate_1d) {
      success_tri = active_tracks_initializer->single_triangulation_1d(
          it1->get(), clones_cam);
    } else {
      success_tri = active_tracks_initializer->single_triangulation(it1->get(),
                                                                    clones_cam);
    }
  ...
  // (retri_rT2-retri_rT1).total_microseconds() * 1e-6); PRINT_DEBUG(CYAN
  // "[RETRI-TIME]: %.4f seconds for triangulate setup\n" RESET,
  // (retri_rT3-retri_rT2).total_microseconds() * 1e-6); PRINT_DEBUG(CYAN
  // "[RETRI-TIME]: %.4f seconds for triangulation\n" RESET,
  // (retri_rT4-retri_rT3).total_microseconds() * 1e-6); PRINT_DEBUG(CYAN
  // "[RETRI-TIME]: %.4f seconds for re-projection\n" RESET,
  // (retri_rT5-retri_rT4).total_microseconds() * 1e-6); PRINT_DEBUG(CYAN
  // "[RETRI-TIME]: %.4f seconds total\n" RESET,
  // (retri_rT5-retri_rT1).total_microseconds() * 1e-6);
}
