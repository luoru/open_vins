...
void UpdaterHelper::get_feature_jacobian_full(
  ...
  // Assert that all the ones in our order are already in our local jacobian
  // mapping
  ...
  for (auto const &pair : feature.timestamps) {
    ...
    for (size_t m = 0; m < feature.timestamps[pair.first].size(); m++) {
      ...
      // possibly the camera intrinsics
...
void UpdaterHelper::nullspace_project_inplace(Eigen::MatrixXd &H_f,
  ...
  // The H_f jacobian max rank is 3 if it is a 3d position, thus size of the
  // left nullspace is Hf.rows()-3 NOTE: need to eigen3 eval here since this
  // experiences aliasing! H_f =
  // H_f.block(H_f.cols(),0,H_f.rows()-H_f.cols(),H_f.cols()).eval();
