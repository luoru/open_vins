...
void UpdaterMSCKF::update(std::shared_ptr<State> state,
  ...
  // NOTE: that when we have the single inverse depth representations, those are
  // only 1dof in size
  ...
  // rT0).total_microseconds() * 1e-6); PRINT_DEBUG("[MSCKF-UP]: %.4f seconds to
  ...
  // rT3).total_microseconds() * 1e-6); PRINT_DEBUG("[MSCKF-UP]: %.4f seconds
  // update state (%d size)\n", (rT5 - rT4).total_microseconds() * 1e-6,
  // (int)res_big.rows()); PRINT_DEBUG("[MSCKF-UP]: %.4f seconds total\n", (rT5
