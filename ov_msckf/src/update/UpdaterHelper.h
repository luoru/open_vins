...
class UpdaterHelper {
 public:
   ...
   * anchored pose)
   */
  static void get_feature_jacobian_full(
      std::shared_ptr<State> state, UpdaterHelperFeature &feature,
   ...
   * This is the MSCKF nullspace projection which removes the dependency on the
   * feature state. Note that this is done **in place** so all matrices will be
   * different after a function call.
   *
   * @param H_f Jacobian with nullspace we want to project onto the system [res
   * = Hx*(x-xhat)+Hf(f-fhat)+n]
   * @param H_x State jacobian
   * @param res Measurement residual
   */
  static void nullspace_project_inplace(Eigen::MatrixXd &H_f,
                                        Eigen::MatrixXd &H_x,
                                        Eigen::VectorXd &res);
   ...
   * Note that this is done **in place** so all matrices will be different after
   * a function call.
   *
   * @param H_x State jacobian
   * @param res Measurement residual
   */
  static void measurement_compress_inplace(Eigen::MatrixXd &H_x,
                                           Eigen::VectorXd &res);
};
