...
bool InertialInitializer::initialize(
  ...
  if (params.init_max_disparity > 0) {
    ...
    FeatureHelper::compute_disparity(_db, average_disparity, variance_disparity,
                                     num_features);
    ...
    if (num_features < 10) {
      PRINT_DEBUG(
          YELLOW
          "[init]: not enough features to compute disparity %d < 10\n" RESET,
          num_features);
      return false;
    }
    ...
    PRINT_DEBUG(
        YELLOW
        "[init]: disparity of the platform is %.4f (%.4f threshold)\n" RESET,
        average_disparity, params.init_max_disparity);
  ...
  if (!disparity_detected_moving && params.init_imu_thresh > 0.0) {
    ...
    return init_dynamic->initialize(timestamp, covariance, order, t_imu,
                                    ...
                                    _calib_IMUtoCAM, _cam_intrinsics);
  }
}
