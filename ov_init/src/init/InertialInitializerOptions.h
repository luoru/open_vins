...
struct InertialInitializerOptions {
   ...
   */
  void print_and_load(
      const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
  ...
  void print_and_load_initializer(
    ...
    if (parser != nullptr) {
      ...
      parser->parse_config("init_dyn_mle_max_threads",
      ...
      parser->parse_config("init_dyn_inflation_ori",
                           init_dyn_inflation_orientation);
      parser->parse_config("init_dyn_inflation_vel",
                           init_dyn_inflation_velocity);
      parser->parse_config("init_dyn_inflation_bg",
                           init_dyn_inflation_bias_gyro);
      parser->parse_config("init_dyn_inflation_ba",
    ...
    if (init_max_features < 15) {
      PRINT_ERROR(
          RED
          "number of requested feature tracks to init not enough!!\n" RESET);
    ...
    if (init_dyn_num_pose < 4) {
      PRINT_ERROR(RED
                  "number of requested frames to init not enough!!\n" RESET);
  ...
  void print_and_load_state(
    ...
    if (num_cameras != (int)camera_intrinsics.size() ||
      ...
      PRINT_ERROR(RED
                  "[SIM]: got %d but expected %d max cameras "
                  "(camera_intrinsics)\n" RESET,
    ...
    for (int n = 0; n < num_cameras; n++) {
      ...
      ss << "cam_" << n << "_wh:" << std::endl
         << camera_intrinsics.at(n)->w() << " x "
         << camera_intrinsics.at(n)->h() << std::endl;
