 ...
 * 3. If the jump is past our threshold we should init (i.e. we have started
 * moving)
 * 4. Use the *previous* window, which should have been stationary to initialize
 * orientation
...
class StaticInitializer {
 public:
   ...
   * If we have then we will use the period of time before this jump to
   * initialize the state. This assumes that our imu is sitting still and is not
   * moving (so this would fail if we are experiencing constant acceleration).
   *
   * In the case that we do not wait for a jump (i.e. `wait_for_jerk` is false),
   * then the system will try to initialize as soon as possible. This is only
   * recommended if you have zero velocity update enabled to handle the
   * stationary cases. To initialize in this case, we need to have the average
   * angular variance be below the set threshold (i.e. we need to be
   * stationary).
   *
