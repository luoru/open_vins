...
bool StaticInitializer::initialize(double &timestamp,
  ...
  if (newesttime - oldesttime < params.init_window_time) {
    PRINT_DEBUG(YELLOW
                "[init-s]: unable to select window of IMU readings, not enough "
                "readings\n" RESET);
    return false;
  }
  ...
  if (a_var_2to1 > params.init_imu_thresh && wait_for_jerk) {
    PRINT_DEBUG(
        YELLOW
        "[init-s]: to much IMU excitation, above threshold %.4f > %.4f\n" RESET,
  ...
  if ((a_var_1to0 > params.init_imu_thresh ||
    ...
    PRINT_DEBUG(YELLOW
                "[init-s]: to much IMU excitation, above threshold %.4f,%.4f > "
                "%.4f\n" RESET,
                a_var_1to0, a_var_2to1, params.init_imu_thresh);
    return false;
  }
  ...
  // picked. This means that on startup, we can fix the yaw and position to be
  // 100 percent known. Thus, after determining the global to current IMU
  // orientation after initialization, we can propagate the global error into
  // the new IMU pose. In this case the position is directly equivalent, but the
  // orientation needs to be propagated.
  ...
  covariance.block(t_imu->q()->id(), t_imu->q()->id(), 3, 3) =
      R_GtoI * covariance.block(t_imu->q()->id(), t_imu->q()->id(), 3, 3) *
      R_GtoI.transpose();
