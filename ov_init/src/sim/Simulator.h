 ...
 *
 * Given a trajectory this will generate a SE(3) @ref ov_core::BsplineSE3 for
 * that trajectory. This allows us to get the inertial measurement information
 * at each timestep during this trajectory. After creating the bspline we will
 * generate an environmental feature map which will be used as our feature
 * measurements. This map will be projected into the frame at each timestep to
 * get our "raw" uv measurements. We inject bias and white noises into our
 * inertial readings while adding our white noise to the uv measurements also.
 * The user should specify the sensor rates that they desire along with the
...
class Simulator {
 public:
   ...
   * @param params_ InertialInitializer parameters. Should have already been
   * loaded from cmd.
   */
