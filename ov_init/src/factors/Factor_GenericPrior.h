 ...
 *
 * This is a general factor which handles state priors which have non-zero
 * linear errors. In general a unitary factor will have zero error when it is
 * created, thus this extra term can be ignored. But if performing
 * marginalization, this can be non-zero. See the following paper Section 3.2
 * Eq. 25-35 https://journals.sagepub.com/doi/full/10.1177/0278364919835021
 *
 ...
 *
 * For example, consider we have the following system were we wish to remove the
 * xm states. This is the problem of state marginalization.
 * @f[
 * [ Arr Arm ] [ xr ] = [ - gr ]
 * @f]
 * @f[
 ...
 * @f]
 *
 * We wish to marginalize the xm states which are correlated with the other
 * states @f$ xr @f$. The Jacobian (and thus information matrix A) is computed
 * at the current best guess @f$ x_{lin} @f$. We can define the following
 * optimal subcost form which only involves the @f$ xr @f$ states as:
 * @f[
 * cost^2 = (xr - xr_{lin})^T*(A^T*A)*(xr - xr_{lin}) + b^T*A*(xr - xr_{lin}) +
 * b^b
 * @f]
 *
 * where we have:
...
class Factor_GenericPrior : public ceres::CostFunction {
 public:
  ...
  /// The square-root of the information s.t. sqrtI^T * sqrtI = marginal
  /// information
  Eigen::MatrixXd sqrtI;

  /// Constant term inside the cost s.t. sqrtI^T * b = marginal gradient (can be
  /// zero)
  Eigen::MatrixXd b;
   ...
   */
  Factor_GenericPrior(const Eigen::MatrixXd &x_lin_,
    ...
    for (auto const &str : x_type_) {
      if (str == "quat") {
        ...
        std::cerr << "type - " << str << " not implemented in prior"
                  << std::endl;
