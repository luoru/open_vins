...
class Factor_ImageReprojCalib : public ceres::CostFunction {
 public:
  ...
  // If value of 1 then this residual adds to the problem, otherwise if zero it
  // is "gated"
  double gate = 1.0;
   ...
   */
  Factor_ImageReprojCalib(const Eigen::Vector2d &uv_meas_, double pix_sigma_,
                          bool is_fisheye_)
   ...
   * This is a function of the observing pose, feature in global, and
   * calibration parameters. The normalized pixel coordinates are found and then
   * distorted using the camera distortion model. See the @ref update-feat page
   * for more details.
   */
  bool Evaluate(double const *const *parameters, double *residuals,
    ...
    if (jacobians) {
      ...
      H_dzn_dpfc << 1.0 / p_FinCi(2), 0, -p_FinCi(0) / std::pow(p_FinCi(2), 2),
          0, 1.0 / p_FinCi(2), -p_FinCi(1) / std::pow(p_FinCi(2), 2);
