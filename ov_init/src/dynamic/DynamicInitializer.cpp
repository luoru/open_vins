...
bool DynamicInitializer::initialize(
  ...
  if (_db->get_internal_data().size() < 0.95 * params.init_max_features) {
    PRINT_WARNING(
        RED
        "[init-d]: only %zu valid features of required (%.0f thresh)!!\n" RESET,
        _db->get_internal_data().size(), 0.95 * params.init_max_features);
    return false;
  }
  if (imu_data->size() < 2 || !have_old_imu_readings) {
    // PRINT_WARNING(RED "[init-d]: waiting for window to reach full size (%zu
    // imu readings)!!\n" RESET, imu_data->size());
    return false;
  }
  ...
  if (count_valid_features < min_valid_features) {
    PRINT_WARNING(RED
                  "[init-d]: only %zu valid features of required %d!!\n" RESET,
                  count_valid_features, min_valid_features);
    return false;
  }

  // Bias initial guesses specified by the launch file
  // We don't go through the effort to recover the biases right now since they
  // should be Semi-well known before launching or can be considered to be near
  // zero...
  ...
  // Feature size of 1 will use the first ever bearing of the feature as true
  // (depth only..)
  ...
  if (num_measurements < system_size) {
    PRINT_WARNING(YELLOW
                  "[init-d]: not enough feature measurements (%d meas vs %d "
                  "state size)!\n" RESET,
  ...
  for (auto const &timepair : map_camera_times) {
    ...
    double cpiI0toIi1_time0_in_imu =
        oldest_camera_time + params.calib_camimu_dt;
    ...
    for (size_t k = 0; k < cpiI0toIi1_readings.size() - 1; k++) {
      ...
      cpiI0toIi1->feed_IMU(imu0.timestamp, imu1.timestamp, imu0.wm, imu0.am,
                           imu1.wm, imu1.am);
    }
  ...
  PRINT_DEBUG(
      "[init-d]: system of %d measurement x %d states created (%d features, "
      "%s)\n",
  ...
  for (auto const &feat : features) {
    ...
    for (auto const &camtime : feat.second->timestamps) {
      ...
      for (size_t i = 0; i < camtime.second.size(); i++) {
        ...
        if (size_feature == 1) {
          assert(false);
          // H.block(0, size_feature * map_features.at(feat.first), 2, 1) =
          // H_proj * R_ItoC * R_I0toIk * features_bearings.at(feat->featid);
        } else {
          H.block(0, size_feature * A_index_features.at(feat.first), 2, 3) =
              H_proj * R_ItoC * R_I0toIk;  // feat
        }
        H.block(0, size_feature * num_features + 0, 2, 3) =
            -H_proj * R_ItoC * R_I0toIk * DT;  // vel
        H.block(0, size_feature * num_features + 3, 2, 3) =
            0.5 * H_proj * R_ItoC * R_I0toIk * DT * DT;  // grav
        Eigen::MatrixXd b_i =
            H_proj * (R_ItoC * R_I0toIk * alpha_I0toIk - p_IinC);

        // Uncertainty of the measurement is the feature pixel noise and and
        // preintegration
        // TODO: We should propagate the raw pixel uncertainty to the normalized
        // and the preintegration error to the measurement
        // TODO: Fix this logic since this uses a unique H_proj matrix
        // structure... Eigen::MatrixXd R_sqrtinv = 1e4 *
        // Eigen::MatrixXd::Identity(2, 2); if (map_camera_cpi_I0toIi.find(time)
        // != map_camera_cpi_I0toIi.end() && map_camera_cpi_I0toIi.at(time) !=
        // nullptr) {
        //   Eigen::MatrixXd H_cpi = H_proj * R_ItoC * R_I0toIk;
        //   Eigen::MatrixXd P_alpha =
        //   map_camera_cpi_I0toIi.at(time)->P_meas.block(12, 12, 3, 3);
        //   Eigen::MatrixXd R = H_cpi * P_alpha * H_cpi.transpose();
        //   R_sqrtinv = R.llt().matrixL();
        //   R_sqrtinv =
        //   R_sqrtinv.colPivHouseholderQr().solve(Eigen::MatrixXd::Identity(R.rows(),
        //   R.rows()));
        // }
        //  Eigen::MatrixXd H_dz_dzn, H_dz_dzeta;
        //  params.camera_intrinsics.at(cam_id)->compute_distort_jacobian(uv_norm,
        //  H_dz_dzn, H_dz_dzeta); Eigen::MatrixXd R_pixel =
        //  std::pow(params.sigma_pix, 2) * H_dz_dzn * H_dz_dzn.transpose();
  ...
  Eigen::MatrixXd A1A1_inv =
      (A1.transpose() * A1)
          .llt()
          .solve(Eigen::MatrixXd::Identity(A1.cols(), A1.cols()));
  Eigen::MatrixXd A2 = A.block(0, A.cols() - 3, A.rows(), 3);
  Eigen::MatrixXd Temp =
      A2.transpose() * (Eigen::MatrixXd::Identity(A1.rows(), A1.rows()) -
                        A1 * A1A1_inv * A1.transpose());
  ...
  Eigen::MatrixXd companion_matrix =
      Eigen::MatrixXd::Zero(coeff.rows() - 1, coeff.rows() - 1);
  ...
  Eigen::JacobiSVD<Eigen::MatrixXd> svd0(
      companion_matrix, Eigen::ComputeThinU | Eigen::ComputeThinV);
  ...
  if (svd0.rank() != companion_matrix.rows()) {
    PRINT_ERROR(RED
                "[init-d]: eigenvalue decomposition not full rank!!\n" RESET);
    return false;
  }
  ...
  if (solver.info() != Eigen::Success) {
    PRINT_ERROR(
        RED
        "[init-d]: failed to compute the eigenvalue decomposition!!\n" RESET);
    return false;
  }

  // Find the smallest real eigenvalue
  // NOTE: we find the one that gives us minimal constraint cost
  // NOTE: not sure if the best, but one that gives the correct mag should be
  // good?
  ...
  if (std::abs(gravity_inI0.norm() - params.gravity_mag) >
    ...
    PRINT_WARNING(YELLOW
                  "[init-d]: gravity did not converge (%.3f > %.3f)\n" RESET,
  ...
  for (auto const &feat : features) {
    ...
    if (size_feature == 1) {
      assert(false);
      // p_FinI0 = x_hat(size_feature * map_features.at(feat.first) + 6, 0) *
      // features_bearings.at(feat->featid);
    } else {
      p_FinI0 = x_hat.block(size_feature * A_index_features.at(feat.first) + 6,
                            0, 3, 1);
    }
  ...
  if (count_valid_features < min_valid_features) {
    PRINT_ERROR(YELLOW
                "[init-d]: not enough features for our mle (%zu < %d)!\n" RESET,
                count_valid_features, min_valid_features);
    return false;
  }
  ...
  for (auto const &timepair : map_camera_times) {
    ...
    if (map_states.empty()) {
      ...
      prior_Info.block(0, 0, 4, 4) *=
          1.0 / std::pow(1e-5, 2);  // 4dof unobservable yaw and position
  ...
  for (auto const &feat : features) {
    ...
    for (auto const &camtime : feat.second->timestamps) {
      ...
      for (size_t i = 0; i < camtime.second.size(); i++) {
        ...
        // If we don't have the feature state we should create that parameter
        // block The initial guess of the features are the scaled feature map
        // from the SFM
  ...
  PRINT_INFO(
      "[init-d]: %d iterations | %zu states, %zu feats (%zu valid) | %d param "
      "and %d res | cost %.4e => %.4e\n",
      (int)summary.iterations.size(), map_states.size(), map_features.size(),
      count_valid_features, summary.num_parameters, summary.num_residuals,
  ...
  // NOTE: for now just the IMU state is recovered, but we should be able to do
  // everything NOTE: maybe having features / clones will make it more stable?
  ...
  options_cov.algorithm_type =
      ceres::CovarianceAlgorithmType::DENSE_SVD;  // SPARSE_QR, DENSE_SVD
  ...
  // block diagonal
  assert(problem_cov.GetCovarianceBlockInTangentSpace(
  ...
  //
  //  // A VIO system has 4dof unobservabile directions which can be arbitrarily
  //  picked.
  //  // This means that on startup, we can fix the yaw and position to be 100
  //  percent known.
  //  // Thus, after determining the global to current IMU orientation after
  //  initialization, we can propagate the global error
  //  // into the new IMU pose. In this case the position is directly
  //  equivalent, but the orientation needs to be propagated. covariance(0, 0) =
  //  1e-3; covariance(1, 1) = 1e-3; covariance(2, 2) = 1e-5;
  //  covariance.block(0, 0, 3, 3) = _imu->Rot() * covariance.block(0, 0, 3, 3)
  //  * _imu->Rot().transpose(); covariance.block(3, 3, 3, 3) = 1e-5 *
  //  Eigen::Matrix3d::Identity();

  // Debug timing information about how long it took to initialize!!
  // PRINT_DEBUG("[TIME]: %.4f sec for prelim tests\n", (rT2 -
  // rT1).total_microseconds() * 1e-6); PRINT_DEBUG("[TIME]: %.4f sec for linsys
  // setup\n", (rT3 - rT2).total_microseconds() * 1e-6); PRINT_DEBUG("[TIME]:
  // %.4f sec for linsys\n", (rT4 - rT3).total_microseconds() * 1e-6);
  // PRINT_DEBUG("[TIME]: %.4f sec for ceres opt setup\n", (rT5 -
  // rT4).total_microseconds() * 1e-6); PRINT_DEBUG("[TIME]: %.4f sec for ceres
  // opt\n", (rT6 - rT5).total_microseconds() * 1e-6); PRINT_DEBUG("[TIME]: %.4f
  // sec for ceres covariance\n", (rT7 - rT6).total_microseconds() * 1e-6);
