 ...
 * Right now we use ORB descriptors as we have found it is the fastest when
 * computing descriptors. Tracks are then rejected based on a ratio test and
 * ransac.
 */
class TrackDescriptor : public TrackBase {
 public:
   ...
   * each other
   * @param knnratio matching ratio needed (smaller value forces top two
   * descriptors during match to be more different)
   */
  explicit TrackDescriptor(
      std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras,
      int numfeats, int numaruco, bool binocular, HistogramMethod histmethod,
 ...
 protected:
   ...
   */
  void feed_stereo(const CameraData &message, size_t msg_id_left,
                   size_t msg_id_right);
   ...
   *
   * Given a set of images, and their currently extracted features, this will
   * try to add new features. We return all extracted descriptors here since we
   * DO NOT need to do stereo tracking left to right. Our vector of IDs will be
   * later overwritten when we match features temporally to the previous frame's
   * features. See robust_match() for the matching.
   */
  void perform_detection_monocular(const cv::Mat &img0, const cv::Mat &mask0,
   ...
   *
   * This does the same logic as the perform_detection_monocular() function, but
   * we also enforce stereo contraints. We also do STEREO matching from the left
   * to right, and only return good matches that are found in both the left and
   * right. Our vector of IDs will be later overwritten when we match features
   * temporally to the previous frame's features. See robust_match() for the
   * matching.
   */
  void perform_detection_stereo(const cv::Mat &img0, const cv::Mat &img1,
                                const cv::Mat &mask0, const cv::Mat &mask1,
                                std::vector<cv::KeyPoint> &pts0,
                                std::vector<cv::KeyPoint> &pts1, cv::Mat &desc0,
                                cv::Mat &desc1, size_t cam_id0, size_t cam_id1,
   ...
   *
   * This will perform a "robust match" between the two sets of points (slow but
   * has great results). First we do a simple KNN match from 1to2 and 2to1,
   * which is followed by a ratio check and symmetry check. Original code is
   * from the "RobustMatcher" in the opencv examples, and seems to give very
   * good results in the matches.
   ...
   */
  void robust_match(std::vector<cv::KeyPoint> &pts0,
                    std::vector<cv::KeyPoint> pts1, cv::Mat &desc0,
                    cv::Mat &desc1, size_t id0, size_t id1,
  ...
  // Minimum pixel distance to be "far away enough" to be a different extracted
  // feature
  int min_px_dist;

  // The ratio between two kNN matches, if that ratio is larger then this
  // threshold then the two features are too close, so should be considered
  // ambiguous/bad match
  double knn_ratio;
