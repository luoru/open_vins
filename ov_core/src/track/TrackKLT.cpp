...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  if (pts_last[cam_id].empty()) {
    // Detect new features
    perform_detection_monocular(imgpyr, mask, pts_last[cam_id],
                                ids_last[cam_id]);
  ...
  perform_detection_monocular(img_pyramid_last[cam_id], img_mask_last[cam_id],
                              pts_last[cam_id], ids_last[cam_id]);
  ...
  perform_matching(img_pyramid_last[cam_id], imgpyr, pts_last[cam_id],
                   pts_left_new, cam_id, cam_id, mask_ll);
  ...
  for (size_t i = 0; i < pts_left_new.size(); i++) {
    ...
    if ((int)message.masks.at(msg_id).at<uint8_t>(
            (int)pts_left_new.at(i).pt.y, (int)pts_left_new.at(i).pt.x) > 127)
      continue;
  ...
  //  rT1).total_microseconds() * 1e-6); PRINT_DEBUG("[TIME-KLT]: %.4f seconds
  //  for detection\n", (rT3 - rT2).total_microseconds() * 1e-6);
  //  PRINT_DEBUG("[TIME-KLT]: %.4f seconds for temporal klt\n", (rT4 -
  //  rT3).total_microseconds() * 1e-6); PRINT_DEBUG("[TIME-KLT]: %.4f seconds
  //  for feature DB update (%d features)\n", (rT5 - rT4).total_microseconds() *
  //  1e-6,
  //              (int)good_left.size());
...
void TrackKLT::feed_stereo(const CameraData &message, size_t msg_id_left,
  ...
  parallel_for_(cv::Range(0, 2), LambdaBody([&](const cv::Range &range) {
                  for (int i = range.start; i < range.end; i++) {
                    ...
                    cv::buildOpticalFlowPyramid(
                        ...
                        is_left ? imgpyr_left : imgpyr_right, win_size,
                        pyr_levels);
                  }
                }));
  ...
  if (pts_last[cam_id_left].empty() && pts_last[cam_id_right].empty()) {
    ...
    perform_detection_stereo(imgpyr_left, imgpyr_right, mask_left, mask_right,
                             cam_id_left, cam_id_right, pts_last[cam_id_left],
                             pts_last[cam_id_right], ids_last[cam_id_left],
  ...
  perform_detection_stereo(
      ...
      cam_id_right, pts_last[cam_id_left], pts_last[cam_id_right],
  ...
  for (size_t i = 0; i < pts_right_new.size(); i++) {
    ...
    bool added_already =
        (std::find(good_ids_right.begin(), good_ids_right.end(),
                   ids_last[cam_id_right].at(i)) != good_ids_right.end());
  ...
  // Timing information
  // PRINT_DEBUG("[TIME-KLT]: %.4f seconds for
  // pyramid\n",(rT2-rT1).total_microseconds() * 1e-6); PRINT_DEBUG("[TIME-KLT]:
  // %.4f seconds for detection\n",(rT3-rT2).total_microseconds() * 1e-6);
  // PRINT_DEBUG("[TIME-KLT]: %.4f seconds for temporal
  // klt\n",(rT4-rT3).total_microseconds() * 1e-6); PRINT_DEBUG("[TIME-KLT]:
  // %.4f seconds for stereo klt\n",(rT5-rT4).total_microseconds() * 1e-6);
  // PRINT_DEBUG("[TIME-KLT]: %.4f seconds for feature DB update (%d
...
void TrackKLT::perform_detection_monocular(const std::vector<cv::Mat> &img0pyr,
  ...
  cv::Size size((int)((float)img0pyr.at(0).cols / (float)min_px_dist),
  ...
  while (it0 != pts0.end()) {
    ...
    if (x_grid < 0 || x_grid >= size.width || y_grid < 0 ||
        y_grid >= size.height || x < 0 || x >= img0pyr.at(0).cols || y < 0 ||
        y >= img0pyr.at(0).rows) {
      it0 = pts0.erase(it0);
      it1 = ids0.erase(it1);
      continue;
  ...
  if (num_featsneeded < std::min(75, (int)(0.2 * num_features))) return;
...
void TrackKLT::perform_detection_stereo(
  ...
  // pixels This means that we will reject points that less then grid_px_size
  // points away then existing features
  cv::Size size0((int)((float)img0pyr.at(0).cols / (float)min_px_dist),
                 (int)((float)img0pyr.at(0).rows / (float)min_px_dist));
  ...
  while (it0 != pts0.end()) {
    ...
    int x_grid = (int)(kpt.pt.x / (float)min_px_dist);
    int y_grid = (int)(kpt.pt.y / (float)min_px_dist);
    if (x_grid < 0 || x_grid >= size0.width || y_grid < 0 ||
        y_grid >= size0.height || x < 0 || x >= img0pyr.at(0).cols || y < 0 ||
        y >= img0pyr.at(0).rows) {
      it0 = pts0.erase(it0);
      it1 = ids0.erase(it1);
      continue;
  ...
  if (num_featsneeded_0 > std::min(75, (int)(0.2 * num_features))) {
    ...
    for (auto &kpt : pts0_ext) {
      ...
      if (x_grid < 0 || x_grid >= size0.width || y_grid < 0 ||
        ...
        continue;
      // See if there is a point at this location
      if (grid_2d_0.at<uint8_t>(y_grid, x_grid) > 127) continue;
      // Else lets add it!
      grid_2d_0.at<uint8_t>(y_grid, x_grid) = 255;
      kpts0_new.push_back(kpt);
      pts0_new.push_back(kpt.pt);
    ...
    if (!pts0_new.empty()) {
      ...
      cv::TermCriteria term_crit = cv::TermCriteria(
          cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 10, 0.01);
  ...
  // location
  cv::Size size1((int)((float)img1pyr.at(0).cols / (float)min_px_dist),
  ...
  while (it0 != pts1.end()) {
    ...
    int x_grid = (int)(kpt.pt.x / (float)min_px_dist);
    int y_grid = (int)(kpt.pt.y / (float)min_px_dist);
    if (x_grid < 0 || x_grid >= size1.width || y_grid < 0 ||
        y_grid >= size1.height || x < 0 || x >= img1pyr.at(0).cols || y < 0 ||
        y >= img1pyr.at(0).rows) {
      it0 = pts1.erase(it0);
      it1 = ids1.erase(it1);
      continue;
  ...
  if (num_featsneeded_1 > std::min(75, (int)(0.2 * num_features))) {
    ...
    Grider_FAST::perform_griding(img1pyr.at(0), mask1, pts1_ext, num_features,
    ...
    for (auto &kpt : pts1_ext) {
      ...
      if (x_grid < 0 || x_grid >= size1.width || y_grid < 0 ||
        ...
        continue;
      // See if there is a point at this location
      if (grid_2d_1.at<uint8_t>(y_grid, x_grid) > 127) continue;
      // Else lets add it!
      pts1.push_back(kpt);
      size_t temp = ++currid;
      ids1.push_back(temp);
...
void TrackKLT::perform_matching(const std::vector<cv::Mat> &img0pyr,
  ...
  cv::findFundamentalMat(pts0_n, pts1_n, cv::FM_RANSAC, 1.0 / max_focallength,
                         0.999, mask_rsc);
