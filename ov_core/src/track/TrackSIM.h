 ...
 *
 * This class should be used when we are using the @ref ov_msckf::Simulator
 * class to generate measurements. It simply takes the resulting simulation data
 * and appends it to the internal feature database.
 */
class TrackSIM : public TrackBase {
 public:
   ...
   */
  TrackSIM(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras,
           int numaruco)
      : TrackBase(cameras, 0, numaruco, false, HistogramMethod::NONE) {}

  /**
   * @brief Process a new image
   * @warning This function should not be used!! Use @ref
   * feed_measurement_simulation() instead.
