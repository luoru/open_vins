 ...
 *
 * This is the implementation of a KLT visual frontend for tracking sparse
...
class TrackKLT : public TrackBase {
 ...
 protected:
   ...
   *
   * Given an image and its currently extracted features, this will try to add
   * new features if needed. Will try to always have the "max_features" being
   ...
   * @param img0pyr left image we will detect features on (first level of
   * pyramid)
   * @param img1pyr right image we will detect features on (first level of
   * pyramid)
   * @param mask0 mask which has what ROI we do not want features in
   ...
   * @param ids0 left vector of feature ids for each currently extracted
   * keypoint
   * @param ids1 right vector of feature ids for each currently extracted
   * keypoint
   *
   * This does the same logic as the perform_detection_monocular() function, but
   * we also enforce stereo contraints. So we detect features in the left image,
   * and then KLT track them onto the right image. If we have valid tracks, then
   * we have both the keypoint on the left and its matching point in the right
   ...
   * The two point vectors will be of equal size, but the mask_out variable will
   * specify which points are good or bad. If the second vector is non-empty, it
