 ...
 * track the corners of the tag as compared to the pose of the tag or any other
 * corners. Right now we hardcode the dictionary to be
 * `cv::aruco::DICT_6X6_1000`, so please generate tags in this family of tags.
 * You can generate these tags using an online utility:
 * https://chev.me/arucogen/ The actual size of the tags do not matter since we
 * do not recover the pose and instead just use this for re-detection and
...
class TrackAruco : public TrackBase {
 public:
   ...
   * it
   * @param numaruco the max id of the arucotags, we don't use any tags greater
   * than this value even if we extract them
   ...
   * screen
   */
  void display_active(cv::Mat &img_out, int r1, int g1, int b1, int r2, int g2,
                      int b2, std::string overlay = "") override;
#endif
 ...
 protected:
  ...
  // Max tag ID we should extract from (i.e., number of aruco tags starting from
  // zero)
  int max_tag_id;
