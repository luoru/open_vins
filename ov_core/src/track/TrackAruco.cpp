...
void TrackAruco::perform_tracking(double timestamp, const cv::Mat &imgin,
  ...
  // Timing information
  // PRINT_DEBUG("[TIME-ARUCO]: %.4f seconds for
  // detection\n",(rT2-rT1).total_microseconds() * 1e-6);
  // PRINT_DEBUG("[TIME-ARUCO]: %.4f seconds for feature DB update (%d
...
void TrackAruco::display_active(cv::Mat &img_out, int r1, int g1, int b1,
  ...
  for (auto const &pair : img_last_cache) {
    ...
    if (!ids_aruco[pair.first].empty())
      cv::aruco::drawDetectedMarkers(img_temp, corners[pair.first],
                                     ids_aruco[pair.first]);
    if (!rejects[pair.first].empty())
      cv::aruco::drawDetectedMarkers(img_temp, rejects[pair.first],
