...
class LandmarkRepresentation {
 public:
   ...
   */
  static inline Representation from_string(
      const std::string &feat_representation) {
    if (feat_representation == "GLOBAL_3D") return GLOBAL_3D;
    if (feat_representation == "GLOBAL_FULL_INVERSE_DEPTH")
      return GLOBAL_FULL_INVERSE_DEPTH;
    if (feat_representation == "ANCHORED_3D") return ANCHORED_3D;
   ...
   */
  static inline bool is_relative_representation(
      Representation feat_representation) {
    return (
