 ...
 * cv::FileStorage object.
 * - From there the user can request for different parameters of different types
 * from the config.
 * - If we have ROS, then we will also check to see if the user has overridden
 * any config files via ROS.
 * - The ROS parameters always take priority over the ones in our config.
 *
 * NOTE: There are no "nested" yaml parameters. They are all under the "root" of
 * the yaml file!!! NOTE: The camera and imu have nested, but those are handled
 * externally....
 */
class YamlParser {
 public:
  ...
  explicit YamlParser(const std::string &config_path,
    ...
    config_path.c_str()); std::exit(EXIT_FAILURE);
    }*/
   ...
   * @param node_result Resulting value (should already have default value in
   * it)
   ...
   * From there it will try to find the first level requested (e.g. imu0, cam0,
   * cam1). Then the requested node can be found under this sensor name. ROS can
   * override the config with `<sensor_name>_<node_name>` (e.g.,
   * cam0_distortion).
   *
   * @tparam T Type of parameter we are looking for.
   * @param external_node_name Name of the node we will get our relative path
   * from
   * @param sensor_name The first level node we will try to get the requested
   * node under
   * @param node_name Name of the node
   * @param node_result Resulting value (should already have default value in
   * it)
  ...
  /**
   * @brief Custom parser for Matrix3d in the external parameter files with
   * levels.
   *
   * This will first load the external file requested.
   * From there it will try to find the first level requested (e.g. imu0, cam0,
  ...
  /**
   * @brief Custom parser for Matrix4d in the external parameter files with
   * levels.
   *
   * This will first load the external file requested.
   * From there it will try to find the first level requested (e.g. imu0, cam0,
 ...
 private:
  ...
  /**
   * @brief This function will try to get the requested parameter from our
   * config.
   *
   * If it is unable to find it, it will give a warning to the user it couldn't
   * be found.
   *
   * @tparam T Type of parameter we are looking for.
   * @param file_node OpenCV file node we will get the data from
  ...
  /**
   * @brief Custom parser for booleans (0,false,False,FALSE=>false and
  ...
  void parse(const cv::FileNode &file_node, const std::string &node_name,
    ...
    try {
      ...
      // NOTE: we select the first bit of text as there can be a comment
      // afterwards NOTE: for example we could have "key: true #comment here..."
      // where we only want "true"
      std::string value;
