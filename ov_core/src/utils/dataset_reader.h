 ...
 * We have some nice utility functions here that handle loading of the
 * groundtruth data. This can be used to initialize the system or for plotting
 * and calculation of RMSE values without needing any alignment.
 *
 * > M. Burri, J. Nikolic, P. Gohl, T. Schneider, J. Rehder, S. Omari,M.
 * Achtelik and R. Siegwart, > "The EuRoC micro aerial vehicle datasets",
 * International Journal of Robotic Research, DOI: 10.1177/0278364915620033,
 * 2016. >
...
class DatasetReader {
 public:
  ...
  static bool get_gt_state(
    ...
    if (gt_states.find(timestep) == gt_states.end()) {
      PRINT_WARNING(YELLOW
                    "Unable to find %.6f timestamp in GT file, wrong GT file "
                    "loaded???\n" RESET,
                    timestep);
      return false;
    }
   ...
   * q_GtoI, p_IinG)
   */
  static void load_simulated_trajectory(
      std::string path, std::vector<Eigen::VectorXd> &traj_data) {
    ...
    while (std::getline(file, current_line)) {
      ...
      while (std::getline(s, field, ' ')) {
        // Skip if empty
        if (field.empty() || i >= data.rows()) continue;
