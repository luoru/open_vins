 ...
 *
 * Each visual tracker has this database in it and it contains all features that
 * we are tracking. The trackers will insert information into this database when
 * they get new measurements from doing tracking. A user would then query this
 * database for features that can be used for update and remove them after they
 * have been processed.
 *
 *
 ...
 * Since each feature is a pointer just directly returning and using them is not
 * thread safe. Thus, to be thread safe, use the "remove" flag for each function
 * which will remove it from this feature database. This prevents the trackers
 * from adding new measurements and editing the feature information. For
 * example, if you are asynchronous tracking cameras and you chose to update the
 * state, then remove all features you will use in update. The feature trackers
 * will continue to add features while you update, whose measurements can be
...
class FeatureDatabase {
 public:
   ...
   * @param remove Set to true if you want to remove the feature from the
   ...
   */
  void update_feature(size_t id, double timestamp, size_t cam_id, float u,
                      float v, float u_n, float v_n) {
  ...
  /**
   * @brief Get features that do not have newer measurement then the specified
   * time.
   *
   * This function will return all features that do not a measurement at a time
   * greater than the specified time. For example this could be used to get
   * features that have not been successfully tracked into the newest frame. All
   * features returned will not have any measurements occurring at a time
   * greater then the specified.
   */
  std::vector<std::shared_ptr<Feature>> features_not_containing_newer(
      double timestamp, bool remove = false, bool skip_deleted = false) {
   ...
   *
   * This will collect all features that have measurements occurring before the
   * specified timestamp. For example, we would want to remove all features
   * older then the last clone/state in our sliding window.
   */
  std::vector<std::shared_ptr<Feature>> features_containing_older(
      double timestamp, bool remove = false, bool skip_deleted = false) {
   ...
   *
   * This function will return all features that have the specified time in
   * them. This would be used to get all features that occurred at a specific
   * clone/state.
   */
  std::vector<std::shared_ptr<Feature>> features_containing(
      double timestamp, bool remove = false, bool skip_deleted = false) {
    // Our vector of old features
   ...
   *
   * If a feature was unable to be used, it will still remain since it will not
   * have a delete flag set
   */
  void cleanup() {
  ...
  /**
   * @brief This function will delete all feature measurements that are older
   * then the specified timestamp
   */
  ...
  /**
   * @brief This function will delete all feature measurements that are at the
   * specified timestamp
   */
  ...
  /**
   * @brief Will update the passed database with this database's latest feature
   * information.
   */
  void append_new_measurements(
        ...
        for (const auto &times : feat.second->timestamps) {
          ...
          if (temp->timestamps.find(cam_id) == temp->timestamps.end()) {
            ...
            for (size_t i = 0; i < feat.second->timestamps.at(cam_id).size();
              ...
              if (std::find(temp_times.begin(), temp_times.end(),
                            time_to_find) == temp_times.end()) {
                temp->timestamps.at(cam_id).push_back(
                    feat.second->timestamps.at(cam_id).at(i));
                temp->uvs.at(cam_id).push_back(
                    feat.second->uvs.at(cam_id).at(i));
